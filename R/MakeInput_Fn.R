...
function(Version, Y, X, Y_Report=NULL, LastRun=NULL, Loc, isPred, ErrorDist, ObsModel, VaryingKappa, n_factors, n_stations, Use_REML, Aniso){
  ...
  # Data
  if(Version=="spatial_factor_analysis_v18") Data = list(Y=Y_fit, Y_Report=Y_Report, NAind=NAind_fit, isPred=isPred, ErrorDist=as.integer(2), VaryingKappa=as.integer(VaryingKappa), n_species=n_species, n_stations=n_fit, n_factors=n_factors, meshidxloc=mesh$idx$loc-1, n_p=ncol(X_fit), X=X_fit, G0=spde$param.inla$M0, G1=spde$param.inla$M1, G2=spde$param.inla$M2)
  if(Version %in% c("spatial_factor_analysis_v19","spatial_factor_analysis_v20") ) Data = list(Pen_Vec=c(0,0), Y=Y_fit, Y_Report=Y_Report, NAind=NAind_fit, isPred=isPred, ErrorDist=as.integer(2), VaryingKappa=as.integer(VaryingKappa), n_species=n_species, n_stations=n_fit, n_factors=n_factors, meshidxloc=mesh$idx$loc-1, n_p=ncol(X_fit), X=X_fit, G0=spde$param.inla$M0, G1=spde$param.inla$M1, G2=spde$param.inla$M2)
  if(Version %in% c("spatial_factor_analysis_v21","spatial_factor_analysis_v22") ) Data = list(Aniso=as.integer(Aniso), Pen_Vec=c(0,0), Y=Y_fit, Y_Report=Y_Report, NAind=NAind_fit, isPred=isPred, ErrorDist=as.integer(2), VaryingKappa=as.integer(VaryingKappa), n_species=n_species, n_stations=n_fit, n_factors=n_factors, n_i=mesh$n, meshidxloc=mesh$idx$loc-1, n_p=ncol(X_fit), X=X_fit, n_tri=nrow(mesh$graph$tv), Tri_Area=Tri_Area, E0=E0, E1=E1, E2=E2, TV=TV-1, G0=spde$param.inla$M0, G0_inv=as(diag(1/diag(spde$param.inla$M0)),"dgTMatrix"), G1=spde$param.inla$M1, G2=spde$param.inla$M2)
  # Parameters
