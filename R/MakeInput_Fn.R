...
function(Version, Y, X, Y_Report=NULL, LastRun=NULL, Loc, isPred, ErrorDist, ObsModel, VaryingKappa, n_factors, n_stations, Use_REML, Aniso, mesh, spde){
  ...
  if(Version %in% c("spatial_factor_analysis_v23") ) Data = list(Aniso=as.integer(Aniso), Options_Vec=ifelse(ObsModel=="Lognorm_Pois",1,0), Pen_Vec=c(0,0), Y=Y, Y_Report=Y_Report, NAind=NAind, isPred=isPred, ErrorDist=ifelse(ObsModel=="Lognorm_Pois",0,2), VaryingKappa=as.integer(VaryingKappa), n_species=n_species, n_stations=n_fit, n_factors=n_factors, n_i=mesh$n, meshidxloc=mesh$idx$loc-1, n_p=ncol(X), X=X, n_tri=nrow(mesh$graph$tv), Tri_Area=Tri_Area, E0=E0, E1=E1, E2=E2, TV=TV-1, G0=spde$param.inla$M0, G0_inv=spde$param.inla$M0_inv, G1=spde$param.inla$M1, G2=spde$param.inla$M2)
  # Parameters
  if(StartValue=="Default" | is.null(LastRun) ){
    ...
    if(ObsModel=="Poisson"){
      ...
      Params[['ln_VarInfl_Lognorm']] = rep(-20,n_species)
    }
    if(ObsModel=="NegBin1"){
    ...
    if(ObsModel=="Lognorm_Pois"){
      ...
      Params[['ln_VarInfl_NB2']] = rep(-20,n_species)
      Params[['ln_VarInfl_ZI']] = rep(-20,n_species)
      Params[['ln_VarInfl_Lognorm']] = rep(-5,n_species)
    }
  }
  if(StartValue=="Last_Run" & !is.null(LastRun)){
    ...
    if(ObsModel=="NegBin12"){
      ...
      Params[['ln_VarInfl_NB2']] = rep(-20,n_species)
      Params[['ln_VarInfl_ZI']] = rep(-20,n_species)
      Params[['ln_VarInfl_Lognorm']] = Par_best[which(names(Par_best)=="ln_VarInfl_Lognorm")]
    }
    if(Version %in% c("spatial_factor_analysis_v20","spatial_factor_analysis_v21") ){
      if(Aniso==0) Params[['ln_H_input']] = c(0,-10,0)
    }
    if(Version %in% c("spatial_factor_analysis_v22","spatial_factor_analysis_v23") ){
  ...
  if(Use_REML==TRUE) Random = c(Random, "beta", "ln_VarInfl_NB1", "ln_VarInfl_NB2", "ln_VarInfl_ZI", "ln_VarInfl_Lognorm")
