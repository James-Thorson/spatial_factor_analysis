  ...
  for(int i=0;i<n_species;i++){
    ...
    for(int j=0;j<n_stations;j++){
      if(NAind(j,i) < 0.5){
        ...
        Type var_y = exp(ln_mean_y(j,i))*(1.0+exp(ln_VarInfl_NB1(i)))+exp(2.0*ln_mean_y(j,i))*exp(ln_VarInfl_NB2(i));
        if(ErrorDist==0){
          g -= dpois( Y(j,i), exp(ln_mean_y(j,i)), 1 ) * (1-isPred(j)); 
          h(i) -= dpois( Y(j,i), exp(ln_mean_y(j,i)), 1 ) * isPred(j); 
        }
        if(ErrorDist==1){
          g -= dnbinom2( Y(j,i), exp(ln_mean_y(j,i)), var_y, 1 ) * (1-isPred(j)); 
          h(i) -= dnbinom2( Y(j,i), exp(ln_mean_y(j,i)), var_y, 1 ) * isPred(j); 
        }
        if(ErrorDist==2){ 
          Tmp = -log( exp(ln_VarInfl_ZI(i)) + 1) + dnbinom2( Y(j,i), exp(ln_mean_y(j,i)), var_y, 1 );
